...
export default function Navbar() {
  ...
  return (
    <header
      className={`sticky top-0 z-50 bg-background px-6 py-5 ${color ? "shadow-[0_0_10px_#94a3b8]" : ""}`}
    >
      <nav className="container mx-auto flex items-center justify-between">
        ...
        <div className="hidden items-center gap-4 text-sm uppercase transition-all duration-150 lg:flex">
          {navLinks.map((link) => {
            const isActive = pathName === link.href;
            return (
              <Link
                key={link.id}
                className={`rounded px-3 py-1 font-medium text-slate-950 transition-colors duration-200 ease-in-out hover:bg-indigo-600 hover:text-white dark:text-foreground ${isActive ? "bg-indigo-700 text-white" : ""}`}
                href={link.href}
              >
                {link.name}
              </Link>
